    ...
    >
      ...
      <div className="relative z-30 flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2 text-[9px] font-mono tracking-widest text-[var(--ui-text-muted)] uppercase">
          <span className="border border-[var(--ui-border)] px-3 py-1 rounded bg-[var(--ui-panel)]/30 backdrop-blur-sm">
            Sys: Online
          </span>
          <span className="border border-[var(--ui-border)] px-3 py-1 rounded bg-[var(--ui-panel)]/30 backdrop-blur-sm shadow-[inset_0_0_8px_rgba(59,130,246,0.2)] text-[var(--status-info)] animate-pulse">
            Scanning
          </span>
        </div>
      </div>
      ...
      <div className="relative z-10 w-full mb-1 aspect-[21/8] bg-black/5 rounded-2xl flex flex-col items-center justify-end overflow-hidden">
        ...
        <div className="absolute top-8 left-10 flex flex-col font-mono text-left z-20 pointer-events-none">
          <div className="text-[var(--ui-text-muted)] tracking-widest uppercase text-[10px] mb-0.5">
            Eng. Temp
          </div>
          <div
            className={`text-2xl font-bold tabular-nums tracking-tight ${showEngineFault ? "text-[var(--status-critical)] drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "text-[var(--ui-text)]"}`}
          >
            {temperatureC.toFixed(1)}
            <span className="text-xs text-[var(--ui-text-muted)] ml-1">°C</span>
          </div>
        ...
        <div className="absolute top-8 right-10 flex flex-col font-mono text-right z-20 pointer-events-none">
          ...
          <div
            className={`text-2xl font-bold tabular-nums tracking-tight ${showElectricalFault ? "text-[var(--status-warning)] drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]" : "text-[var(--ui-text)]"}`}
          >
            {voltageV}
            <span className="text-xs text-[var(--ui-text-muted)] ml-1">V</span>
          </div>
        ...
        <div className="absolute bottom-16 right-10 flex flex-col font-mono text-right z-20 pointer-events-none">
          ...
          <div
            className={`text-2xl font-bold tabular-nums tracking-tight ${showBrakeFault ? "text-[var(--status-purple)] drop-shadow-[0_0_8px_rgba(168,85,247,0.5)]" : "text-[var(--ui-text)]"}`}
          >
            {brakePressureKpa.toFixed(0)}
            <span className="text-xs text-[var(--ui-text-muted)] ml-1">
              kPa
            </span>
          </div>
          <div className="w-16 h-px mt-1 ml-auto bg-gradient-to-l from-[var(--ui-text)] to-transparent opacity-30" />
        ...
        <div className="absolute bottom-0 w-[200%] h-8 opacity-40 group-hover:opacity-80 transition-opacity duration-1000 [mask-image:linear-gradient(to_right,transparent,black_20%,black_80%,transparent)]">
          ...
          <div className="absolute inset-0 flex items-center gap-10 animate-[scroll-track_0.5s_linear_infinite]">
            {Array.from({ length: 40 }).map((_, idx) => (
              <div
                key={idx}
                className="h-4 w-[2px] bg-[var(--ui-text)]/70 transform -skew-x-[25deg] shadow-[1px_0_3px_rgba(255,255,255,0.2)]"
              />
            ))}
          </div>
        </div>
